...
class Rule_H004(BaseRule):
    """The WHERE keyword must be isolated by itself in a signle line.
    ...
    **Best practice**
    ...
    def _eval(self, context: RuleContext) -> Optional[LintResult]:
        ...
        for segment in context.segment.recursive_crawl_all():
            if isinstance(segment, KeywordSegment) and segment.raw.upper() == 'WHERE':
                where_segment = segment

            if where_segment is not None and isinstance(segment, WhitespaceSegment):
